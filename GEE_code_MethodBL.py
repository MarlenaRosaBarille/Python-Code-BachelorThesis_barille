...
Original file is located at
    https://colab.research.google.com/drive/1eGrHJ0na_ApwbErnwy9NbVS504xTggy-
...
"""## 5. LS Factor Baseline (BL) = combined LS factor by Moore & Wilson 1992

Moore & Wilson (1992). Length-slope factors for the Revised Universal Soil Loss Equation: Simplified method of estimation. Journal of Soil and
Water Conservation, 47 (5), 423–428. https://doi.org/10.1080/00224561.1992.12456740
...
#MOORE combined LS Factor


# LS = (m + 1) * (SCA / 22.13)^m * (sin(slope) / 0.0896)^n
#Parameters
M_EXP = 0.4
N_EXP = 1.3

LS = (
    ee.Image.constant(M_EXP + 1.0)
    .multiply(
        sca30.divide(22.13).pow(M_EXP)
    )
    .multiply(
        sin_slope.divide(0.0896).pow(N_EXP)
    )
).rename('LS')
...
Map.addLayer(LS, {'min': 0, 'max': 20}, 'LS factor')

Map

#Export of final image asset LS Factorc "LS_F2" to then ingest to Arcgis via "gee-toolbox" for mapping and analysis
asset_id = "projects/rusle-476614/assets/LS_BL_clip"
...
With standard RUSLE units, $A_y$ is in **t·ha⁻¹·yr⁻¹**.
...
#Export of final image asset "A_rusle" to then ingest to Arcgis via "gee-toolbox".
# Get CRS string
...
asset_id = "projects/rusle-476614/assets/RUSLE_BL"

target_crs = target_proj.crs().getInfo()
target_transform = dem.projection().getInfo()['transform']   # or dem30.projection().getInfo()['transform']

task = ee.batch.Export.image.toAsset(
    image=A_rusle_BL.toFloat(),
    description="RUSLE_BL_export",
    assetId=asset_id,
    region=aoi,
    crs=target_crs,
